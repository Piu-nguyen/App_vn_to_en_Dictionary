...
public:
    ...
    void saveWord(Word& w) {
        ...
        auto it = find_if(savedWords.begin(), savedWords.end(), [&](const auto& sw){ return sw.english == w.english; });
        if (it != savedWords.end()) {
            it->exp += 10;
            it->isNew = false;
            it->learned = true;
            w.learned = true;
            cout << "Cập nhật: " << w.english << " (+10 EXP) [" << w.topic << "]\n";
        } else {
            w.exp = 10;
            w.isNew = true;
            w.learned = true;
            savedWords.push_back(w);
            cout << "Đã lưu: " << w.english << " (MỚI +10 EXP) [" << w.topic << "]\n";
        }
        saveWordsToFile();
    }

    void deleteSavedWord(const string& eng) {
        if (!isLoggedIn) { cout << "Cần đăng nhập!\n"; return; }
        auto it = remove_if(savedWords.begin(), savedWords.end(), [&](const Word& w) { return w.english == eng; });
        if (it != savedWords.end()) {
            savedWords.erase(it, savedWords.end());
            saveWordsToFile();
            cout << "Đã xóa: " << eng << "\n";
        } else {
            cout << "Không tìm thấy: " << eng << "\n";
        }
    }
    ...
    bool isWordSaved(const string& eng) const {
        return any_of(savedWords.begin(), savedWords.end(), [&](const auto& w){ return w.english == eng; });
    }
};

// ===============================
// GRAMMAR TRAINER – CHỈ ĐỌC & RA 10 CÂU
// ===============================
...
public:
    ...
    void practiceTest() {
        ...
        int testIdx = uniform_int_distribution<int>(0, static_cast<int>(practiceTests.size()) - 1)(g);
    ...
    // CHỈ ĐỌC → LẤY 10 CÂU ĐẦU → TRỘN → RA
    void learnGrammars() {
        while (true) {
            cout << "\n=== HỌC 12 THÌ ===\n";
...
private:
    ...
    int findByNameOrNum(const vector<string>& items, const string& prompt) {
        while (true) {
            string input = safeInput(prompt);
            input = trim(input);
            if (input == "0") return 0;
            if (isNumber(input)) {
                ...
                if (idx >= 0 && idx < (int)items.size()) return idx + 1;
            } else {
                string n = normalize(input);
                for (int i = 0; i < (int)items.size(); ++i)
                    if (normalize(items[i]) == n) return i + 1;
            }
            cout << "Không hợp lệ. Nhập lại (0 để thoát).\n";
        }
    }

    void askToSaveWord(Word* w) {
        if (!account.isLogin()) return;
        if (account.isWordSaved(w->english)) {
            cout << "Từ \"" << w->english << "\" đã được lưu trước đó.\n";
            return;
        }
        string choice = safeInput("\nLưu từ này vào danh sách cá nhân? (y/n): ");
        if (!trim(choice).empty() && tolower(trim(choice)[0]) == 'y') {
            account.saveWord(*w);
        } else {
            cout << "Không lưu từ.\n";
        }
...
public:
    void buildData() {
        allWords.clear(); topics.clear(); dict = Dictionary();
        fs::path dataPath = fs::current_path() / "Data";

        if (!fs::exists(dataPath)) {
            cout << "Tạo thư mục 'Data' và file mẫu...\n";
            ...
            sample << "cat,mèo,/kæt/,The cat is black.,animals\n";
            sample << "bird,chim,/bɜːd/,Birds can fly.,animals\n";
            sample.close();
        }
        ...
        cout << "Tải thành công " << total << " từ từ " << topics.size() << " chủ đề!\n\n";
    }
    ...
    void viewTopic() {
        if (topics.empty()) { cout << "Chưa có dữ liệu.\n"; return; }
        showTopics();
        int choice = findByNameOrNum(topics, "\nChọn chủ đề (0 để thoát): ");
        if (choice <= 0) return;

        string selected = topics[choice - 1];
        vector<string> wordList;
        for (const auto& w : allWords)
            if (w.topic == selected) wordList.push_back(w.english);
        ...
        while (true) {
            int idx = findByNameOrNum(wordList, "\nChọn từ (0 để thoát): ");
            if (idx <= 0) break;

            auto it = find_if(allWords.begin(), allWords.end(), [&](const Word& w){ return w.english == wordList[idx - 1]; });
            if (it == allWords.end()) continue;

            it->learned = true;
            saveHistory(it->english);

            cout << "\nCHI TIẾT TỪ\n";
            cout << "Từ: " << it->english << "\n";
            cout << "Nghĩa: " << it->vietnamese << "\n";
            cout << "Phiên âm: " << it->ipaText << "\n";
            cout << "Ví dụ: " << it->example << "\n";
            cout << "Chủ đề: " << it->topic << "\n";
    ...
    void searchWord() {
        ...
        Word* w = dict.findEng(input);
        if (!w) w = dict.findVie(input);

        if (w) {
            auto it = find_if(allWords.begin(), allWords.end(), [&](const Word& ww){ return ww.english == w->english; });
            if (it != allWords.end()) it->learned = true;
    ...
    void menu() {
        ...
        do {
            ...
            switch (choice) {
                ...
                case 9: if (account.isLogin()) grammar.practiceTest(); else cout << "Đăng nhập trước!\n"; break;
            }
        } while (true);
